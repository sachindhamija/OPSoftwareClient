...
interface SaleRegisterProps {
  ...
  saleRegisterParams?: SaleRegisterParams | null;
}

const SaleRegister = ({ isInModal = false, saleRegisterParams = null, onSuccessfulSubmit }: SaleRegisterProps) => {
  ...
  useEffect(() => {
    if (financialYear?.financialYearFrom && !initialDatesSet) {
      ...
      fetchSaleRegisterData(startDate, endDate);
    }
  ...
  const fetchSaleRegisterData = async (fromDate: string, toDate: string) => {
    try {
      ...
      setSaleRegisterEntries(response);
      setTotals(newTotals);
    } catch (error) {
      toast.error('Failed to fetch sale register data.');
      console.error(error);
    }
  };
  ...
  const handleFilterSubmit = async (data: FieldValues) => {
    ...
    await fetchSaleRegisterData(data.fromDate, data.toDate);
  ...
  const handleCloseModal = () => {
    ...
    fetchSaleRegisterData(fromDate,toDate);
    if (isInModal && saleRegisterParams != null && onSuccessfulSubmit) {
      onSuccessfulSubmit();
    }
  };
          ...
          <Row>
            <div className="custom-table-container">
              <Table striped bordered hover className="mt-3 custom-table">
                ...
                <tbody>
                  {saleRegisterEntries.map((entry, index) => (

...
const TrialBalanceReport = () => {
	...
	const [selectedAccount, setSelectedAccount] =
		useState<LedgerReportParams | null>(null);
	const [ledgerModalVisible, setLedgerModalVisible] =
	...
	const [trialBalanceData, setTrialBalanceData] =
		useState<TrialBalanceDto | null>(null);

	const fetchTrialBalanceData = async (toDate: string) => {
		if (financialYear?.financialYearFrom !== undefined) {
			try {
				const data = await agent.Reports.getTrailBalanceReport(
					accessId,
					formatDateForBackend(financialYear?.financialYearFrom),
	...
	const handleRowClick = (accountId: string) => {
		if (validateDate(toDate) && financialYear?.financialYearFrom) {
			setSelectedAccount({
				accountId,
				fromDate: financialYear?.financialYearFrom,
				toDate: toDate,
			});
		...
		<>
				...
				{trialBalanceData && (
					<div style={{ maxHeight: '550px', overflowY: 'auto' }}>
						...
						>
							...
							<tbody>
								{trialBalanceData.accountGroups.map(
									(group: TrialBalanceAccountGroupsDto) => (
										<React.Fragment
										...
										>
											...
											>
												...
												>
													{formatNumberIST(
														group.creditTransactionBalance
													)}
												</td>
												<td
													style={{
											...
											{group.accounts.map((account) => (
												...
												>
													...
													<td className="text-end">
														{formatNumberIST(
															account.debitOpeningBalance
														)}
													</td>
													<td className="text-end">
														{formatNumberIST(
															account.creditOpeningBalance
														)}
													</td>
													<td className="text-end">
														{formatNumberIST(
															account.debitTransactionBalance
														)}
													</td>
													<td className="text-end">
														{formatNumberIST(
															account.creditTransactionBalance
														)}
													</td>
													<td className="text-end">
														{formatNumberIST(
															account.debitClosingBalance
														)}
													</td>
													<td className="text-end">
														{formatNumberIST(
															account.creditClosingBalance
														)}
													</td>
												</tr>
											))}
							...
							<tfoot className="table-footer">
								<tr className="grand-total">
									...
									<td className="text-end">
										<strong>
											{formatNumberIST(
												trialBalanceData.grandTotalDebitOpening
											)}
										</strong>
									</td>
									<td className="text-end">
										<strong>
											{formatNumberIST(
												trialBalanceData.grandTotalCreditOpening
											)}
										</strong>
									</td>
									<td className="text-end">
										<strong>
											{formatNumberIST(
												trialBalanceData.grandTotalDebitTransaction
											)}
										</strong>
									</td>
									<td className="text-end">
										<strong>
											{formatNumberIST(
												trialBalanceData.grandTotalCreditTransaction
											)}
										</strong>
									</td>
									<td className="text-end">
										<strong>
											{formatNumberIST(
												trialBalanceData.grandTotalDebitClosing
											)}
										</strong>
									</td>
									<td className="text-end">
										<strong>
											{formatNumberIST(
												trialBalanceData.grandTotalCreditClosing
											)}
										</strong>
									</td>
								</tr>
								<tr className="differences">
									...
									<td className="text-end" colSpan={2}>
										<strong>
											{
												trialBalanceData.openingBalanceDifference
											}
										</strong>
									</td>
									<td className="text-end" colSpan={4}>
										<strong>
											{
												trialBalanceData.closingBalanceDifference
											}
										</strong>
									</td>
								</tr>
			...
			>
				{selectedAccount && (
					<LedgerReport
						isInModal={true}
						ledgerParams={selectedAccount}
						onSuccessfulSubmit={() => fetchTrialBalanceData(toDate)}
					/>
				)}
			</CommonModal>
		</>

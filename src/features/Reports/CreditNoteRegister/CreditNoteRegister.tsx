...
export interface CreditNoteRegisterParams {
  fromDate: Date;
  toDate: Date;
}
...
interface CreditNoteRegisterProps {
  ...
  creditNoteRegisterParams?: CreditNoteRegisterParams | null;
}

const CreditNoteRegister = ({ isInModal = false, creditNoteRegisterParams = null, onSuccessfulSubmit }: CreditNoteRegisterProps) => {
  const accessId = getAccessIdOrRedirect();
  const financialYear = useAppSelector(selectCurrentFinancialYear);
  const [creditNoteRegisterEntries, setCreditNoteRegisterEntries] = useState<ItemRegisterDto[]>([]);
  ...
  useEffect(() => {
    if (financialYear?.financialYearFrom && !initialDatesSet) {
      ...
      fetchCreditNoteRegisterData(startDate, endDate);
    }
  ...
  const fetchCreditNoteRegisterData = async (fromDate: string, toDate: string) => {
    try {
      ...
      setCreditNoteRegisterEntries(response);
      setTotals(newTotals);
    } catch (error) {
      toast.error('Failed to fetch credit note register data.');
      console.error(error);
    }
  };
  ...
  const handleFilterSubmit = async (data: FieldValues) => {
    ...
    await fetchCreditNoteRegisterData(data.fromDate, data.toDate);
  ...
  const handleCloseModal = () => {
    ...
    fetchCreditNoteRegisterData(fromDate,toDate);
    if (isInModal && creditNoteRegisterParams != null && onSuccessfulSubmit) {
      onSuccessfulSubmit();
    }
  };
          ...
          <Row>
            <div className="custom-table-container">
              <Table striped bordered hover className="mt-3 custom-table">
                ...
                <tbody>
                  {creditNoteRegisterEntries.map((entry, index) => (

...
export interface SaleReturnRegisterParams {
  fromDate: Date;
  toDate: Date;
}

interface SaleReturnRegisterProps {
  isInModal?: boolean;
  onSuccessfulSubmit?: () => void;
  saleReturnRegisterParams?: SaleReturnRegisterParams | null;
}

const SaleReturnRegister = ({ isInModal = false, saleReturnRegisterParams = null, onSuccessfulSubmit }: SaleReturnRegisterProps) => {
  const accessId = getAccessIdOrRedirect();
  const financialYear = useAppSelector(selectCurrentFinancialYear);
  const [saleReturnRegisterEntries, setSaleReturnRegisterEntries] = useState<ItemRegisterDto[]>([]);
  ...
  useEffect(() => {
    if (financialYear?.financialYearFrom && !initialDatesSet) {
      ...
      fetchSaleReturnRegisterData(startDate, endDate);
    }
  ...
  const fetchSaleReturnRegisterData = async (fromDate: string, toDate: string) => {
    try {
      ...
      setSaleReturnRegisterEntries(response);
      setTotals(newTotals);
    } catch (error) {
      toast.error('Failed to fetch sale return register data.');
      console.error(error);
    }
  };
  ...
  const handleFilterSubmit = async (data: FieldValues) => {
    ...
    await fetchSaleReturnRegisterData(data.fromDate, data.toDate);
  ...
  const handleCloseModal = () => {
    ...
    fetchSaleReturnRegisterData(fromDate,toDate);
    if (isInModal && saleReturnRegisterParams != null && onSuccessfulSubmit) {
      onSuccessfulSubmit();
    }
  };
          ...
          <Row>
            <div className="custom-table-container">
              <Table striped bordered hover className="mt-3 custom-table">
                ...
                <tbody>
                  {saleReturnRegisterEntries.map((entry, index) => (
...
export default SaleReturnRegister;
...
import { SalePurchaseForm } from '../../Vouchers/Common Vouchers Form/CommonForm';
...
const LedgerReport = ({
	...
	const [selectedVoucher, setSelectedVoucher] =
		useState<LedgerReportDto | null>(null);
	...
	const fetchLedgerData = async (
		...
		try {
			...
			const response = await agent.Reports.getLedgerReport(
				accessId,
				accountId,
			...
			if (ledgerParams != null && isInModal) {
				...
				setAccountOptions((prevOptions) => {
					const selectedAccountOption = prevOptions.find(
						(option) => option.value === ledgerParams.accountId
					);
				...
				await fetchLedgerData(
					ledgerParams.accountId,
					ledgerParams.fromDate.toString(),
					ledgerParams.toDate.toString()
				);
			}
		};

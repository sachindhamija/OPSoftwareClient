...
export interface PurchaseReturnRegisterParams {
  fromDate: Date;
  toDate: Date;
}

interface PurchaseReturnRegisterProps {
  isInModal?: boolean;
  onSuccessfulSubmit?: () => void;
  purchaseReturnRegisterParams?: PurchaseReturnRegisterParams | null;
}

const PurchaseReturnRegister = ({ isInModal = false, purchaseReturnRegisterParams = null, onSuccessfulSubmit }: PurchaseReturnRegisterProps) => {
  const accessId = getAccessIdOrRedirect();
  const financialYear = useAppSelector(selectCurrentFinancialYear);
  const [purchaseReturnRegisterEntries, setPurchaseReturnRegisterEntries] = useState<ItemRegisterDto[]>([]);
  ...
  useEffect(() => {
    if (financialYear?.financialYearFrom && !initialDatesSet) {
      ...
      fetchPurchaseReturnRegisterData(startDate, endDate);
    }
  ...
  const fetchPurchaseReturnRegisterData = async (fromDate: string, toDate: string) => {
    try {
      ...
      const response = await agent.Reports.ItemRegister(
        ...
        VoucherTypeEnum.PurchaseReturn
      );
      ...
      setPurchaseReturnRegisterEntries(response);
      setTotals(newTotals);
    } catch (error) {
      toast.error('Failed to fetch purcahse return register data.');
      console.error(error);
    }
  };
  ...
  const handleFilterSubmit = async (data: FieldValues) => {
    ...
    await fetchPurchaseReturnRegisterData(data.fromDate, data.toDate);
  ...
  const handleCloseModal = () => {
    ...
    fetchPurchaseReturnRegisterData(fromDate,toDate);
    if (isInModal && purchaseReturnRegisterParams != null && onSuccessfulSubmit) {
      onSuccessfulSubmit();
    }
  };
  ...
  return (
    <>
      <CommonCard 
        header="Purchase Return Register" 
        size="100%"
      >
          ...
          <Row>
            <div className="custom-table-container">
              <Table striped bordered hover className="mt-3 custom-table">
                ...
                <tbody>
                  {purchaseReturnRegisterEntries.map((entry, index) => (

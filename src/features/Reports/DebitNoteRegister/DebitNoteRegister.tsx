...
export interface DebitNoteRegisterParams {
  fromDate: Date;
  toDate: Date;
}
...
interface DebitNoteRegisterProps {
  ...
  debitNoteRegisterParams?: DebitNoteRegisterParams | null;
}

const DebitNoteRegister = ({ isInModal = false, debitNoteRegisterParams = null, onSuccessfulSubmit }: DebitNoteRegisterProps) => {
  ...
  useEffect(() => {
    if (financialYear?.financialYearFrom && !initialDatesSet) {
      ...
      fetchDebitNoteRegisterData(startDate, endDate);
    }
  ...
  const fetchDebitNoteRegisterData = async (fromDate: string, toDate: string) => {
    try {
      ...
      setDebitNoteRegisterEntries(response);
      setTotals(newTotals);
    } catch (error) {
      toast.error('Failed to fetch debit note register data.');
      console.error(error);
    }
  };
  ...
  const handleFilterSubmit = async (data: FieldValues) => {
    ...
    await fetchDebitNoteRegisterData(data.fromDate, data.toDate);
  ...
  const handleCloseModal = () => {
    ...
    fetchDebitNoteRegisterData(fromDate,toDate);
    if (isInModal && debitNoteRegisterParams != null && onSuccessfulSubmit) {
      onSuccessfulSubmit();
    }
  };
          ...
          <Row>
            <div className="custom-table-container">
              <Table striped bordered hover className="mt-3 custom-table">
                ...
                <tbody>
                  {debitNoteRegisterEntries.map((entry, index) => (

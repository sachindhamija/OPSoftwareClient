...
import { ItemShortageEntryDto } from "./ItemShortageEntryDto";
...
function CreditSaleEntry({
  ...
  >([]);
  const [entriesList, setEntriesList] = useState<ItemShortageEntryDto[]>([]);
  ...
  const columns: ColumnDef<ItemShortageEntryDto>[] = [
    {
  ...
  const onSubmit = async (data: ItemShortageEntryDto) => {
    const isUpdate = !!currentVoucherId;
    try {
      data.voucherDate = formatDateForBackend(data.voucherDate);
      const numericFields: (keyof ItemShortageEntryDto)[] = [
        "amount",
        "expense",
        "netAmount",
      ];
  ...
  function handleEdit(row: ItemShortageEntryDto): void {
    if (row.voucherId) {
  ...
  return (
    <>
      <CommonCard header="Item Shortage Entry" size="100%">
        <FormNavigator
        ...
        >
          <Row>
            ...
            <CustomInput
              ...
              value={type === "shortage" ? "-" : type === "increased" ? "+" : ""}
              disabled={true}
            />
          </Col>
            <Col xs={4} md={2}>

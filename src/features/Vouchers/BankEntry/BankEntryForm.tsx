...
const EntryTypes = [
	{
		label: 'Cheque Issued/Neft/Rtgs/UPI',
		value: 'Cheque Issued/Neft/Rtgs/UPI',
	},
	{
		label: 'Cheque Received/Neft/Rtgs/UPI',
		value: 'Cheque Received/Neft/Rtgs/UPI',
	},
	...
	{
		label: 'Interest Received from Saving Acc.',
		value: 'Interest Received from Saving Acc.',
	},
];
...
function BankEntryForm({
	...
	const onSubmit = async (data: BankEntryDto) => {
		...
		try {
			...
			const numericFields: (keyof BankEntryDto)[] = [
				'amount',
				'expense',
				'netAmount',
			];
				...
				if (voucher) {
					...
					setAccountOptions((prevOptions) => {
						...
						if (selectedAccountOption) {
							setValue(
								'accountId',
								selectedAccountOption.accountID
							);
						}
						return prevOptions;
					});
					setBankAccountOptions((prevOptions) => {
						const selectedBankAccount = prevOptions.find(
							(option) =>
								option.accountID === voucher.bankAccountId
						);
					...
					<Row>
						...
						<Col xs={12} md={4}>
							<CustomDropdown
								label="Bank Name"
								name="bankAccountId"
								options={bankAccountOptions.map(

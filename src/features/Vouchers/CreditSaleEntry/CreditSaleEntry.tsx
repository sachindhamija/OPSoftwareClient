...
import { CreditSaleEntryDto } from "./CreditSaleEntryDto";
...
function CreditSaleEntry({
  ...
  const {
    ...
    reset,
  } = useForm<CreditSaleEntryDto>({
    mode: "all",
  });
  const [showAccountModal, setShowAccountModal] = useState(false);
  const [, setBankAccountOptions] = useState<
    AccountDtoForDropDownList[]
  >([]);
  const [accountOptions, setAccountOptions] = useState<
    AccountDtoForDropDownList[]
  >([]);
  const [entriesList, setEntriesList] = useState<CreditSaleEntryDto[]>([]);
  ...
  const [isAccountDropdownDisabled, setIsAccountDropdownDisabled] =
    useState(false);
  ...
  const columns: ColumnDef<CreditSaleEntryDto>[] = [
    {
  ...
  const fetchEntriesAsPerDate = async () => {
    try {
      ...
      if (dateToUse) {
        ...
        setEntriesList(response.entries.map(entry => ({ ...entry, billNo: Number(entry.billNo) || 0 })));
  ...
  const onSubmit = async (data: CreditSaleEntryDto) => {
    const isUpdate = !!currentVoucherId;
    try {
      data.voucherDate = formatDateForBackend(data.voucherDate);
      const numericFields: (keyof CreditSaleEntryDto)[] = [
        "amount",
        "expense",
        "netAmount",
      ];
      ...
      if (isUpdate) {
        ...
        await agent.BankEntry.saveBankEntry(accessId, { ...processedData, billNo: processedData.billNo.toString() });
  ...
  function handleEdit(row: CreditSaleEntryDto): void {
    if (row.voucherId) {

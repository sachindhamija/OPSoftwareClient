...
function PaymentAndReceiptForm({
	voucherType,
	voucherId = undefined,
	isInModal = false,
	onSuccessfulSubmit,
}: PaymentAndReceiptFormProps) {
	...
	const fetchAccounts = async (currentVoucherDate: Date | string) => {
		...
		try {
			const accounts =
				await agent.PaymentAndReceipt.getAccountsForDropDownListPaymentOrReceipt(
					accessId,
	...
	const fetchEntriesAsPerDate = async () => {
		try {
			...
			if (dateToUse) {
				...
				const response =
					await agent.PaymentAndReceipt.getAllPaymentOrReceiptVouchersByDate(
						accessId,
						formattedDate,
						voucherType
					);
	...
	const onSubmit = async (data: PaymentAndReceiptDto) => {
		try {
			...
			data.voucherDate = formatDateForBackend(
				data.voucherDate.toString()
			);
			data.voucherType = voucherType;
			const numericFields: (keyof PaymentAndReceiptDto)[] = [
				'basicAmount',
				'discountAmount',
				'netAmount',
			];
			...
			if (currentVoucherId) {
				await agent.PaymentAndReceipt.savePaymentOrReceipt(
					accessId,
					processedData,
					currentVoucherId
				);
	...
	const handleDelete = async () => {
		if (voucherType !== undefined && currentVoucherId !== undefined) {
			if (
				window.confirm('Are you sure you want to delete this voucher?')
			) {
				try {
					await agent.Vouchers.delete(
						accessId,
						currentVoucherId,
						voucherType
					);
	...
	useEffect(() => {
		const getVoucherById = async () => {
			...
			try {
				const voucher =
					await agent.PaymentAndReceipt.getPaymentOrReceiptVoucherById(
						accessId,
						voucherId
					);
				if (voucher) {
					...
					setAccountOptions((prevOptions) => {
						const selectedAccountOption = prevOptions.find(
							(option) => option.value === voucher.accountId
						);
					...
					setValue(
						'voucherDate',
						formatDateForFrontend(newVoucherDate)
					);
			...
			>
				<CommonCard
					header={
						voucherType == undefined
							? ''
							: getVoucherTypeString(voucherType)
					}
					size="100%"
					onControlPanelClick={() => {

...
import { CommissionAgentItemDto } from './commissionAgentItem';
...
function CommissionAgentItemForm() {
    const {
        register,
        handleSubmit,
        setValue,
        ...
        reset,
        control
    } = useForm<CommissionAgentItemDto>();
    const accessId = getAccessIdOrRedirect();
    const dispatch = useAppDispatch();
    const [items, setItems] = useState<CommissionAgentItemDto[]>([]);
    const [isEditMode, setIsEditMode] = useState(false);
    const [editingItem, setEditingItem] = useState<CommissionAgentItemDto | null>(null);

    const columns: ColumnDef<CommissionAgentItemDto>[] = [
        {
    ...
    const onSubmit = async (data: CommissionAgentItemDto) => {
        dispatch(setLoading(true));
        const numericFields: (keyof CommissionAgentItemDto)[] = [
            'weightPerBag',
            'utraiPerBag',
            'chhanaiPerBag',
            'tulaiPerBag',
    ...
    const handleEdit = (row: CommissionAgentItemDto) => {
        setEditingItem(row);
        setIsEditMode(true);
    ...
    const handleDelete = async (row: CommissionAgentItemDto) => {
        if (row.id === undefined) {
    ...
    return (
        <CommonCard header="Commission Agent Item" size="100%">
...
export default CommissionAgentItemForm;

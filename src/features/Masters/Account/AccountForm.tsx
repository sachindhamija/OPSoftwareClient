...
import {
	useAccountGroups,
	useDebitCreditOptions,
	useGstSlabs,
	useStates,
} from '../../../app/hooks/indexOptionsHooks';
import {
	CapitalAccountFields,
	DefaultFields,
	FarmerFields,
	FixedAssetFields,
	TradingAccountFields,
} from './indexAccountFormFields';
...
import '../../Masters/Account/Account.scss';
...
const PARTY_TYPES_OPTIONS = [
	...
	{
		label: 'Un-Registered Out of State',
		value: 'Un-Registered Out of State',
	},
	{
		label: 'Out of Country (GST Not Applicable)',
		value: 'Out of Country (GST Not Applicable)',
	},
	{
		label: 'Out of Country (GST Applicable)',
		value: 'Out of Country (GST Applicable)',
	},
];
...
function AccountForm({
	...
	const renderGroupSpecificFields = () => {
		const lowerCaseGroup =
			typeof selectedAccountGroup === 'string'
				? selectedAccountGroup.toLowerCase()
				: '';
		if (
			lowerCaseGroup.includes('bank account') ||
			lowerCaseGroup.includes('secured loans') ||
			lowerCaseGroup.includes('expenses')
		) {
			return <></>;
	...
	const onSubmit = async (data: AccountDto) => {
		try {
			...
			if (data.partyType) {
				if (
					(data.partyType.includes('GST Party') ||
						data.partyType.includes('Composition')) &&
					(!data.gstNo || data.gstNo.length != 15)
				) {
					toast.error(
						'Please enter GSTNo. or change the party type if no GSTNo.'
					);
					return;
				}
			}
			...
			const financialYearFrom =
				financialYear?.financialYearFrom.toString();
			const defaultAccountData = getDefaultFormData();
			const finalData = { ...defaultAccountData, ...data };

			if (!finalData.stateId) {
				const companyState = stateOptions.find((option) =>
					option.label.includes(data.companyState)
				);
				finalData.companyState = companyState && companyState?.value;
			}

			const numericFields: (keyof AccountDto)[] = [
				'openingBalance',
				'opUnit',
				'opWeight',
				'opRate',
				...
				'cessPer',
				'capitalSalary',
				'lastYearGrossProfitPercentage',
				'marginPercentage',
				'discountPercentage',
			];
			const processedData = convertNullOrEmptyToZero(
				finalData,
				numericFields
			);
			const fieldsToProcessForNull: (keyof AccountDto)[] = [
				'cityId',
				'stateId',
				'gstSlabId',
				'itemUnitId',
			...
			if (accountId) {
				await agent.Account.updateAccount(
					accessId,
					accountId,
					financialYearFrom,
					finalProcessedData
				...
				await agent.Account.createAccount(
					accessId,
					financialYearFrom,
					finalProcessedData
				);
	...
	useEffect(() => {
		const fetchAccountDetails = async () => {
			...
			if (accountId && accountGroupOptions.length > 0) {
				try {
					const accountDetails = await agent.Account.getAccountById(
						accessId,
						accountId
					);
					reset(accountDetails);
					const selectedAccountGroupOption = accountGroupOptions.find(
						(option) =>
							option.value === accountDetails.accountGroupID
					);
					selectedAccountGroupOption &&
						setSelectedAccountGroup(
							selectedAccountGroupOption?.label
						);
					selectedAccountGroupOption &&
						setValue(
							'accountGroupID',
							selectedAccountGroupOption.value
						);
					const selectStateOption = stateOptions.find(
						(option) =>
							option.value === accountDetails.stateId?.toString()
					);
					selectStateOption &&
						setValue('stateId', selectStateOption.value);
					const measurementsOptions = MEASUREMENTS_OPTIONS.find(
						(option) =>
							option.value.includes(
								accountDetails?.measurement as string
							)
					);
	...
	return (
		<>
			<Container className="mt-3 mt-md-0 mt-lg-0 create-account-container">
				<CommonCard
				...
				>
					...
					>
						...
						<Row>
							<Col
								xs={12}
								className="d-flex mt-3 align-items-end justify-content-end mb-2"
							>
								<CustomButton

...
function ItemUnitForm() {
	...
	const onSubmit = async (data: ItemUnitDto) => {
		...
		try {
			if (
				isEditMode &&
				editingUnit &&
				editingUnit.itemUnitID !== undefined
			) {
				await agent.ItemUnit.updateItemUnit(
					accessId,
					editingUnit.itemUnitID,
					data
				);
	...
	const handleDelete = async (row: ItemUnitDto) => {
		...
		if (
			window.confirm(
				`Are you sure you want to delete the Item Unit "${row.unitName}"?`
			)
		) {
			dispatch(setLoading(true));
			try {
				...
				toast.success(
					`Item Unit "${row.unitName}" deleted successfully`
				);
				getAllItemUnits();
			} catch (error) {

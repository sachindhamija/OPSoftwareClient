...
function ItemCompanyForm() {
	const {
		...
		reset,
	} = useForm<ItemCompanyDto>();
	const accessId = getAccessIdOrRedirect();
	const dispatch = useAppDispatch();
	const [itemCompanies, setItemCompanies] = useState<ItemCompanyDto[]>([]);
	const [isEditMode, setIsEditMode] = useState(false);
	const [editingItemCompany, setEditingItemCompany] =
		useState<ItemCompanyDto | null>(null);
	...
	const getAllItemCompanies = async () => {
		...
		try {
			const fetchedItemCompanies =
				await agent.ItemCompany.getAllItemCompanies(accessId);
	...
	const onSubmit = async (data: ItemCompanyDto) => {
		...
		try {
			if (accessId) {
				if (
					editingItemCompany &&
					editingItemCompany.itemCompanyID !== undefined
				) {
					await agent.ItemCompany.updateItemCompany(
						accessId,
						editingItemCompany.itemCompanyID,
						data
					);
	...
	const handleDelete = async (row: ItemCompanyDto) => {
		...
		if (
			window.confirm(
				`Are you sure you want to delete the item company "${row.itemCompanyName}"?`
			)
		) {
			dispatch(setLoading(true));
			try {
				await agent.ItemCompany.deleteItemCompany(
					accessId,
					row.itemCompanyID
				);
				toast.success(
					`Item Company "${row.itemCompanyName}" deleted successfully`
				);
				getAllItemCompanies();
			} catch (error) {
				...
				<Row>
					<Col xs={12} md={10}>
						<CustomInput
							...
							register={register}
							validationRules={{
								required: 'Item ompany Name cannot be empty.',
							}}
						/>
					</Col>
					<Col

...
function GSTSlabForm({ isModalOpen = false, onSaveSuccess }:GSTSlabFormProps) {
	...
	const [editingGSTSlab, setEditingGSTSlab] = useState<GSTSlabDto | null>(
		null
	);
	...
	const getAllGSTSlabs = async () => {
		...
		try {
			const fetchedGSTSlabs = await agent.GSTSlab.getAllGSTSlabs(
				accessId
			);
	...
	const onSubmit = async (data: GSTSlabDto) => {
		dispatch(setLoading(true));
		const numericFields: (keyof GSTSlabDto)[] = [
			'sgst',
			'cgst',
			'igst',
			'additionalTax',
		...
		try {
			if (
				isEditMode &&
				editingGSTSlab &&
				editingGSTSlab.gstSlabID !== undefined
			) {
				await agent.GSTSlab.updateGSTSlab(
					accessId,
					editingGSTSlab.gstSlabID,
					processedData
				);
	...
	const handleDelete = async (row: GSTSlabDto) => {
		...
		if (
			window.confirm(
				`Are you sure you want to delete the GST Slab "${row.gstSlabName}"?`
			)
		) {
			dispatch(setLoading(true));
			try {
				...
				toast.success(
					`GST Slab "${row.gstSlabName}" deleted successfully`
				);
				getAllGSTSlabs();
			} catch (error) {
				...
				<Row>
					<Col xs={12}>
						<CustomInput
							...
							register={register}
							validationRules={{
								required: 'GST Slab Name cannot be empty.',
							}}
						/>
					</Col>
				</Row>

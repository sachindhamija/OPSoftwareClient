...
function ItemCategoryForm() {
	...
	const [editingItemCategory, setEditingItemCategory] =
		useState<ItemCategoryDto | null>(null);
	const {
		register,
		handleSubmit,
		setValue,
	...
	const fetchItemCategories = async () => {
		...
		try {
			const data = await agent.ItemCategory.getAllItemCategories(
				accessId
			);
	...
	const onSubmit = async (data: ItemCategoryDto) => {
		...
		try {
			if (editingItemCategory?.itemCategoryID) {
				await agent.ItemCategory.updateItemCategory(
					accessId,
					editingItemCategory.itemCategoryID,
					data
				);
	...
	const handleDelete = async (row: ItemCategoryDto) => {
		...
		if (
			window.confirm(
				`Are you sure you want to delete the item Category "${row.itemCategoryName}"?`
			)
		) {
			dispatch(setLoading(true));
			try {
				await agent.ItemCategory.deleteItemCategory(
					accessId,
					row.itemCategoryID
				);
				toast.success(
					`Item Category "${row.itemCategoryName}" deleted successfully`
				);
				fetchItemCategories();
			} catch (error) {

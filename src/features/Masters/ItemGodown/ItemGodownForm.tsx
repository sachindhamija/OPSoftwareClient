...
function ItemGodownForm() {
	const {
		...
		reset,
	} = useForm<ItemGodownDto>();
	const accessId = getAccessIdOrRedirect();
	const dispatch = useAppDispatch();
	const [itemCompanies, setItemCompanies] = useState<ItemGodownDto[]>([]);
	const [isEditMode, setIsEditMode] = useState(false);
	const [editingItemGodown, setEditingItemGodown] =
		useState<ItemGodownDto | null>(null);
	...
	const getAllItemGodowns = async () => {
		...
		try {
			const fetchedItemCompanies =
				await agent.ItemGodown.getAllItemGodowns(accessId);
	...
	const onSubmit = async (data: ItemGodownDto) => {
		...
		try {
			if (accessId) {
				if (
					editingItemGodown &&
					editingItemGodown.itemGodownID !== undefined
				) {
					await agent.ItemGodown.updateItemGodown(
						accessId,
						editingItemGodown.itemGodownID,
						data
					);
	...
	const handleDelete = async (row: ItemGodownDto) => {
		...
		if (
			window.confirm(
				`Are you sure you want to delete the item godown "${row.itemGodownName}"?`
			)
		) {
			dispatch(setLoading(true));
			try {
				await agent.ItemGodown.deleteItemGodown(
					accessId,
					row.itemGodownID
				);
				toast.success(
					`Item Godown "${row.itemGodownName}" deleted successfully`
				);
				getAllItemGodowns();
			} catch (error) {

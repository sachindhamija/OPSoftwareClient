...
function AccountGroupForm() {
	...
	const getAllAccountGroups = async () => {
		...
		try {
			const fetchedGroups = await agent.AccountGroup.getAllAccountGroups(
				accessId
			);
	...
	useEffect(() => {
		const getUnderGroup = async () => {
			try {
				const underGroups = await agent.AccountGroup.getAllUnderGroups(
					accessId
				);
				const formattedUnderGroups = underGroups.map(
					(underGroup: any) => ({
		...
		try {
			if (accessId) {
				if (editingGroup && editingGroup.groupID !== undefined) {
					await agent.AccountGroup.update(
						accessId,
	...
	const handleDelete = async (row: AccountGroupDto) => {
		...
		if (
			window.confirm(
				`Are you sure you want to delete the group "${row.groupName}"?`
			)
		) {
			dispatch(setLoading(true));
			try {
				...
				<Row>
					<Col xs={12}>
						<CustomInput
							...
							register={register}
							validationRules={{
								required: 'Account Group Name cannot be empty.',
							}}
						/>
					</Col>
				</Row>
